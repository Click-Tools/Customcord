/**
* -~-~-~ Main ${config.appName} Patch Script -~-~-~
    ...
    switch (index) {
        case 0: {
            ...
            await M.Main(async () => {
                ...
                await Shell.runSilently(`zip -q -r Dist/${config.appName}-${ipaName.split("_")[1]}_GGSans-Font.ipa Payload & wait $!`, async (stderr, _) => {
                ...
                for (const Font of ipaList.filter(ipa => ipa !== 'GGSans')) {
                    await Shell.runSilently(`unzip -qq -o Dist/${config.appName}-${ipaName.split("_")[1]}_GGSans-Font.ipa`);
                    await Shell.runSilently(`cp -rf Fonts/woff2/${Font}/* Payload/Discord.app/`);
                    await Shell.runSilently(`zip -q -r Dist/${config.appName}-${ipaName.split("_")[1]}_${Font}-Font.ipa Payload & wait $!`);
                    await Shell.runSilently(`rm -rf Payload & wait $!`);
...
const main = async () => {
    ...
    await Shell.runSilently(`plutil -replace CFBundleDisplayName -string "${config.appName}" ${MAIN_PLIST} & wait $!`, (stderr) => {
        Shell.write(stderr
    ...
    await Shell.write(`${S.SUCCESS} Successfully built ${M.PINK}${config.appName}${M.GREEN} in ${M.CYAN}${(END_TIME - START_TIME) / 1000} seconds${M.GREEN}.`);
};
await main();
